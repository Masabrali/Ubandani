                ...
                dbRef.onSnapshot( (music) => {
                    ...
                    music.docChanges.forEach( (change) => {
                        ...
                        if (change.type == "added")
                            return ( addedMusic[change.doc.id] = { ...change.doc.data(), path: change.doc.ref.path } );
                        else if (change.type == "modified")
                            return ( changedMusic[change.doc.id] = { ...change.doc.data(), path: change.doc.ref.path } );
                        else if (change.type == "removed")
                            return ( removedMusic[change.doc.id] = { ...change.doc.data(), path: change.doc.ref.path } );
                    } );
                            ...
                            if (!isEmpty(isConnected.errors)) return errorHandler(isConnected.errors[0]);
                                ...
                                return (
	                            	dbRef.get().then( (music) => {
                    					...
                    					music.forEach( (music) => ( _music[music.id] = { ...music.data(), path: music.ref.path } ) );
